...
class TestReadSlate:
    ...
    async def test_read_slate_with_existing_content(
        ...
        mock_logfire.span.assert_called_once_with(
            "read_slate", user_id=str(run_context.deps.user_id)
        )
        mock_log_tool_call.assert_called_once_with(run_context, "read_slate", {})
        mock_logfire.info.assert_called_once_with(
            "Slate content retrieved", content_length=len(test_slate.content)
        )
    ...
    async def test_read_slate_multiple_slates_returns_most_recent(
        ...
        older_slate = Slate(
            user_id=test_user.id, content="Older content", updated_at=older_time
        )
        ...
        newer_slate = Slate(
            user_id=test_user.id, content="Newer content", updated_at=newer_time
        )
...
class TestUpdateSlate:
    ...
    async def test_update_existing_slate(
        ...
        mock_logfire.info.assert_called_with(
            "Slate updated", slate_id=str(test_slate.id)
        )
    ...
    async def test_create_new_slate_when_none_exists(
        ...
        existing_slates = (
            db_session.query(Slate).filter(Slate.user_id == test_user.id).all()
        )
    ...
    async def test_update_slate_logging(
        ...
        # Assert logging
        mock_logfire.span.assert_called_once_with(
            "update_slate", user_id=str(run_context.deps.user_id)
        )
        mock_log_tool_call.assert_called_once_with(
            run_context, "update_slate", input_data.model_dump()
        )
