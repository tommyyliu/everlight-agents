...
def test_user(db_session, sample_user_id):
    ...
    user = User(
        id=sample_user_id,
        firebase_user_id="test_firebase_id",
...
def mock_embed_query():
    ...
    with patch('ai.tools.data.embed_query') as data_mock, \
         patch('ai.tools.notes.embed_query') as notes_mock:
...
def test_slate(db_session, test_user):
    ...
    slate = Slate(
        user_id=test_user.id,
        content="Test slate content for testing purposes"
    )
...
def test_raw_entry(db_session, test_user):
    ...
    raw_entry = RawEntry(
        user_id=test_user.id,
        source="test_source",
        content={"text": "Test raw entry content that is quite long and will be truncated in the formatted output when displayed to users"},
        embedding=embedding
    )
