...
class RawEntry(Base):
    ...
    source: Mapped[str] = mapped_column(
        String(100)
    )  # e.g., 'journal', 'voice_note', 'import', etc.
    content: Mapped[dict] = mapped_column(
        JSON
    )  # JSONB-like storage for flexible content structure
    embedding: Mapped[NDArray[np.float16]] = mapped_column(
        HALFVEC(3072)
    )  # Store embedding vector (768 dimensions for Gemini)
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())
...
class IntegrationToken(Base):
    ...
    token_metadata: Mapped[Optional[dict]] = mapped_column(
        JSON
    )  # Additional token info (expires_at, scope, etc.)
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())
...
class Note(Base):
    ...
    )  # Agent that created the summary
    title: Mapped[str] = mapped_column(String(255))
