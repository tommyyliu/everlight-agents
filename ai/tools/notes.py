...
async def create_note(ctx: RunContext[AgentContext], input_data: CreateNoteInput) -> str:
    """Create and store a structured summary with semantic embedding."""
    with logfire.span("create_note", title=input_data.title, user_id=str(ctx.deps.user_id)):
        log_tool_call(ctx, "create_note", input_data.model_dump())
        
        db = next(get_db_session())
        
        agent_record = db.query(DBAgent).filter(
        ...
        logfire.info("Note created successfully", note_id=str(note.id), title=input_data.title)
        return f"Note created successfully with ID: {note.id}"


async def update_note(ctx: RunContext[AgentContext], input_data: UpdateNoteInput) -> str:
    """Update an existing note's content and optionally title."""
    with logfire.span("update_note", note_id=input_data.note_id, user_id=str(ctx.deps.user_id)):
...
async def search_notes(ctx: RunContext[AgentContext], input_data: NoteSearchInput) -> str:
    ...
    with logfire.span("search_notes", query=input_data.query, limit=input_data.limit):
        ...
        stmt = select(Note).where(Note.user_id == ctx.deps.user_id).order_by(
...
async def get_note_titles(ctx: RunContext[AgentContext]) -> str:
    ...
    with logfire.span("get_note_titles", user_id=str(ctx.deps.user_id)):
        ...
        db = next(get_db_session())
        notes = db.query(Note).filter(Note.user_id == ctx.deps.user_id).order_by(Note.created_at.desc()).all()
