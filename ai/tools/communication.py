...
async def send_message_tool(
    ctx: RunContext[AgentContext], input_data: SendMessageInput
) -> str:
    ...
    with logfire.span("send_message_tool", channel=input_data.channel):
        ...
        if local_mode:
            ...
            return (
                f"Message queued for delivery to {input_data.channel} via Cloud Tasks."
            )


async def schedule_message(
    ctx: RunContext[AgentContext], input_data: ScheduleMessageInput
) -> str:
    """Schedule a message to be sent at a specific time."""
    with logfire.span(
        "schedule_message",
        channel=input_data.channel,
        run_at=input_data.run_at.isoformat(),
    ):
