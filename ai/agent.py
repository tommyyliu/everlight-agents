...
class AgentFactory:
    ...
    def create_agent_from_db(
        ...
        agent_config = db.query(DBAgent).filter(
...
async def run_agent_from_db(
    ...
    with logfire.span("db_agent_run", user_id=str(user_id), agent_name=agent_name):
        ...
        logfire.info("Database agent run completed", response_length=len(result.data), agent_name=agent_name)
        return result.data
...
def get_agent_config(user_id: UUID, agent_name: str) -> Optional[DBAgent]:
    """Get agent configuration from database"""
    db = next(get_db_session())
    return db.query(DBAgent).filter(
        DBAgent.user_id == user_id,
        DBAgent.name == agent_name
    ).first()
